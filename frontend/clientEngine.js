...
socket.on('stream-scene', (data) => {
    BABYLON.SceneLoader.Append('', data, scene, () => {
        console.log('scene acquire client');
        initScene();
    });
});

socket.on('load-scene', (url, location) => {
...
function initScene() {
    ...
    var camera = new BABYLON.ArcRotateCamera("arcCamera", Math.PI / 2, Math.PI / 3 , 20, new BABYLON.Vector3(0,0,5), scene);
    ...
    var ambientLight_Up_Down = new BABYLON.HemisphericLight('ambientLight', new BABYLON.Vector3(0, 1, 0), scene);
    var ambientLight_Down_Up = new BABYLON.HemisphericLight('ambientLight', new BABYLON.Vector3(0, -1, 0), scene);
